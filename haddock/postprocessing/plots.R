...
N_dist <- dist.alignment(N_alignment) %>% 
  ...
  # select(n_protein, `dist_from_OC43-N_A`)
...
## Line Chart
ggplot(experiment_results %>% filter(!n_protein == "OC43-N" ),
       aes(
         # x = factor(n_protein, variant_order),
         x = `dist_from_OC43-N_A`,
           y = prodigy_deltaG_kcalpermol,
  ...
  stat_cor(method = "spearman",
           aes(color = "black"),
           label.x = mean(experiment_results$`dist_from_OC43-N_A`),
           # label.x = 1,

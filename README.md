...
## Data Prep
Experiment inputs are generated by following the steps in the [inputs/make_task_grip.ipynb](inputs/make_task_grip.ipynb) Jupyter notebook. This creates an experiment folder for each combination of a SARS-CoV-2 [inputs/N-Proteins](N protein) and a human [inputs/Cytokines](Cytokine), generating the appropriate HADDOCK inputs such as the `run.param` file, AIR restraint files, and the `run-docking.csh` script.
